...
description: Run SQL queries and manage transactions on Amazon Aurora Serverless and RDS clusters with Data API enabled using awsclaw. Execute statements, batch operations, and transaction management.
---
...
- **RDS Data → RDS**: Use `RDSTool` `DescribeDBClusters` to find cluster ARNs and check if Data API is enabled (see awsclaw-rds skill)
